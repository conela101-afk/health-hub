...
- **Simple mode** — a sitewide, persistent toggle (top bar) for sensory overload, panic, or executive dysfunction: bigger tap targets, higher-contrast text, and the crisis/out-of-hours banners open by default. The only thing this app stores in your browser is this one on/off preference.
...
- **No persisted personal health data, and no app-generated clinical or legal judgments.** Features like "Before your appointment" (`#/prep`) only ever echo back what the user typed — nothing is saved to storage, and the app never infers, suggests, autocompletes, or characterises (e.g. "this breaches standard X") anything about their care. If a future feature needs to persist personal health/legal content on-device (a saved patient summary, a communication log, a medication log), that's a deliberate scope change, not a small addition — it needs its own privacy/threat-model pass (a shared device, a lost phone, someone else reading it) before any storage code goes in, not just "encrypt it."

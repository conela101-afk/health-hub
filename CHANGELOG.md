...
- Depth-checked the specialties `coverage-gap-tracker.md` had left at "5, therefore adequate" without verifying what those 5 entries actually were — the same blind spot the Cardiology build-out (6 Sep) exposed. Found two genuine gaps and fixed them:
  ...
  - **Genetics**: had zero Northern Ireland coverage. Added the NI Regional Genetics Service, Belfast City Hospital (5 → 6).
  - **Older Persons**: covered dementia/memory clinics well but had no named general geriatric-medicine department. Added St James's Medicine for the Elderly (MedEl, including a Falls & Blackout Unit) and Beaumont's Department of Geriatric Medicine (6 → 8).
  - **Allergy & Immunology**: checked and left as-is — already has adult (St James's), paediatric (CHI), and NI (Regional Immunology Service) clinical entries plus an honest note about the genuine absence of a national adult-allergy charity. Padding it further would have been for its own sake, not the user's.
  - **Crisis**: already substantially rebuilt in the 6-9 Sep session (6 named inpatient psychiatric units added) — not revisited this pass.
- All new/changed entries verified via live web search (HSE/hospital sites, Belfast Trust, Lois Bridges' own site) and cross-checked switchboard numbers already verified elsewhere in `data.js`.
- `data.js`: 484 → 490 entries.

## 2026-09-12 [Claude]
- **New feature: "Search a condition" link-out directory** (`#/conditions`, `#/conditions/:id`), built from a supplied research doc (`Health_Hub_Condition_Directory__All-Island_Source_Verification_and_Link-Out_Architecture.md`) proposing a three-tier condition-information layer (official A-Z, disease charities, medication PILs). Kept as a standalone layer — new `data/conditions.js` (`CONDITIONS`, `CONDITION_OFFICIAL_LINKS`, `MEDICINE_SEARCH_TARGETS`), new render functions in `app.js`, no change to `data.js`/`ENTRIES` — same treatment the Facilities layer got, since this maps a condition name to *who explains it*, not to a local service.
  - 104 conditions seeded from the doc's Section E target list, categorised (Cardiology, Respiratory, Endocrinology, Rheumatology, Neurology, Gastroenterology, Renal, Oncology, Mental Health, Rare/Genetic, Dermatology, Allergy/Immunology), with a search/filter index page.
  - **Real constraint, same as the Phase D facilities build:** this session's outbound network access was fully blocked (every external domain tested — HSE, NHS, emc, MHRA — returned `EGRESS_BLOCKED`/`CONNECT_REJECTED`), so nothing could be re-verified live from here. Only the 13 conditions where the source doc wrote out a **complete, already-confirmed URL** (not just a slug pattern) got a condition-specific deep link: heart attack, angina, atrial fibrillation, heart failure, heart valve disease, congenital heart disease, COPD, type 1 diabetes, rheumatoid arthritis, osteoarthritis, gout, fibromyalgia, motor neurone disease. No slug was guessed or extrapolated for the other 91 — see `data/conditions.js`'s header comment.
  - Every condition's page still resolves to *something* live regardless: a universal "official directories" block (HSE.ie conditions index, nidirect A-Z, an NHS.uk site search for the condition name) renders on every detail page, satisfying the doc's Tier-1 "guaranteed landing page" goal without fabricating exact-match URLs.
  - **New feature: medicine-leaflet search** (`#/medicines`) — a live user-typed query against HPRA, medicines.ie, emc, MHRA and the NHS medicines A-Z, using the query-string patterns the source doc confirmed (`?query=`, `?q=`, `?search=`). No per-drug verification needed since nothing is stored — the site's own search box resolves it at click time.
  - Added quick-link pills to the home page (both Calm mode and full nav) for both new pages; `sw.js` cache bumped to v7 and now precaches `data/conditions.js`.
  - Verified in-browser (Playwright against a local static server): home → pills → conditions index → filter "gout" → detail page (verified links + fallback links) → asthma detail (fallback links only, no fabricated deep link) → medicines page with an empty vs. filled query. No console errors beyond expected Google Fonts/egress-blocked network noise.
  - **Not done, flagged for a future session with real network access:** re-verify and extend condition-specific deep links toward the doc's full multi-hundred-condition and Tier-2/Tier-3 charity vision — see `MASTER-BUILD-PLAN.md`.
